...
# (1001)HDU7457.深度自同构
## 题意
求由 $n$ 个结点构成，且满足“深度相同的结点，度相同”的森林的方案数。

## 解题思路
先考虑 $i$ 个结点的合法的树，这棵树每个结点的子树形态相同。
去掉根结点，每个部分的子树形态相同，那么每个子树的结点数是 $i-1$ 的约数。
设 $f(i)$ 表示 $i$ 个结点满足上述条件的树的方案数，那么有：$f(i)=\sum_{d|(i-1)} f(d)$

再考虑 $n$ 个结点的合法的森林，每个树的结点数是 $n$ 的约数。
设 $g(n)$ 表示 $n$ 个结点满足上述条件的森林的方案数，那么有：$g(n)=\sum_{d|n} f(d)$
...
$1$ 直接跳到其他结点的方案是很优的：
若结点 $i$ 不在结点 $1$ 所在的连通块内，那么 $1$ 直接跳到 $i$ 的代价只可能是 $i$ （$i$ 为奇数）或 $i+1$ （$i$ 为偶数）。

但如果存在 $E(1,2,0)$ ，对于 $x=2,6,10...$ 这一类满足 $x\%4=2$ 的结点， $1$ 直接跳到 $x$ 的代价为 $x+1$ ，但先走到 $2$ 再跳到 $x$ 的代价仅为 $x$ 。

因此，对于 $x\%4=2$ 的结点，需要加入 $E(2,x,k(x|2))$ 。

同理，对于 $x\%2^{i+1}=2^i$ 的结点，需要加入 $E(2,x,k(x|2^i))$ 。
